...
// ---------- Modal ----------
  ...
  const SETTINGS_ACTIVE_CLASS = "vc-settings-active";
  const MENU_PREP_CLASS = "vc-menu-prep";

  
  // Optional: hitbox debug overlay (no console needed)
  // Add ?hitbox=1 to the URL to show cyan outlines for mapped hit areas.
  try{
    const params = new URLSearchParams(location.search);
    if(params.has("hitbox")){
      document.body.classList.add("vc-hitbox-debug");
    }
  }catch(e){}
function setGlobalScrollLock(locked) {
  ...
  function setScreen(name) {
    ...
    if (body) {
      ...
      body.classList.toggle(SETTINGS_ACTIVE_CLASS, name === "settings");
    }
    ...
    if (name === "menu") {
      ...
      // Ensure Settings always opens at top (prevents hitbox drift)
      if (el.screensRoot) el.screensRoot.scrollTop = 0;
    } else {
      if (body) body.classList.remove(MENU_PREP_CLASS);
    }
