...
// Route B: disable the legacy/custom modal overlay pass.
// We keep the markup in place (and can re-enable later), but for now
// use native alerts so the modal can never trap touch input on iOS.
const USE_CUSTOM_MODAL = false;
...
// ---------- Modal ----------
  function showModal(title, text) {
    // Legacy/custom modal overlay can trap touch input if anything goes wrong.
    // Route B: keep saves and other flows working, but use native alerts.
    if (!USE_CUSTOM_MODAL) {
      // Ensure any previously visible modal is forcibly dismissed.
      if (el?.modalBackdrop) el.modalBackdrop.hidden = true;
      const t = title || "Notice";
      const msg = (text || "").trim();
      // iOS Safari: keep it simple and reliable.
      window.alert(msg ? `${t}\n\n${msg}` : t);
      return;
    }
