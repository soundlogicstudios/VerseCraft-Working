...
const CATALOG_URL = CONTENT_ROOT + "Catalog.json";
  ...
  // --- Modal safety: disable any accidental legacy/duplicate modal backdrops ---
  // If a duplicate #modalBackdrop sneaks in during merges, iOS Safari can get "modal locked"
  // because our code targets the first element with that id. We always target the backdrop
  // that actually contains the modal panel, and hide any stray overlays.
  function normalizeModalDom() {
    const backdrops = Array.from(document.querySelectorAll("#modalBackdrop"));
    if (!backdrops.length) return;
    const primary =
      backdrops.find((d) => d && (d.querySelector("#modalPanel") || d.querySelector("#btnModalClose") || d.querySelector("#modalTitle"))) ||
      backdrops[backdrops.length - 1];
    el.modalBackdrop = primary || el.modalBackdrop;
    // Hide any stray/legacy overlays so they cannot block touches.
    backdrops.forEach((d) => {
      if (d && d !== el.modalBackdrop) d.hidden = true;
    });
  }
  normalizeModalDom();
...
// ---------- Modal ----------
  function showModal(title, text) {
    ...
    // Hide any stray/legacy modal backdrops first
    Array.from(document.querySelectorAll('#modalBackdrop')).forEach((d) => { if (d && d !== el.modalBackdrop) d.hidden = true; });
    if (el.modalBackdrop) el.modalBackdrop.hidden = false;
  }

  function hideModal() {
    // Hide ALL modal backdrops (fixes duplicate-id legacy overlays)
    Array.from(document.querySelectorAll('#modalBackdrop')).forEach((d) => { if (d) d.hidden = true; });
  }
...
updateContinueButton();
  ...
  // ---------- Revision 1.0: Hitbox Debug Overlay (separate layer) ----------
  let vcHitboxDebugOn = false;
  const vcHitboxIds = [
    "btnSettingsBack",
    "btnClearSave",
    "cosmeticsThemeSelect"
  ];

  function ensureHitboxLayer(){
    let layer = document.getElementById("vcHitboxDebugLayer");
    if (!layer){
      layer = document.createElement("div");
      layer.id = "vcHitboxDebugLayer";
  ...
  function refreshHitboxDebug(){
    if (!vcHitboxDebugOn) return;
    const layer = ensureHitboxLayer();
    layer.classList.add("on");
    layer.innerHTML = "";

    vcHitboxIds.forEach((id) => {
      const el = document.getElementById(id);
      if (!el) return;
      const r = el.getBoundingClientRect();
      // Ignore offscreen/hidden
      if (r.width <= 0 || r.height <= 0) return;

      const box = document.createElement("div");
      box.className = "vc-hitbox-box";
      ...
      const label = document.createElement("div");
      label.className = "vc-hitbox-label";
      // Show computed % if present in inline style; otherwise show "css"
      const cs = window.getComputedStyle(el);
      const top = cs.top || "css";
      const height = cs.height || "css";
      const left = cs.left || "css";
      const width = cs.width || "css";
      label.textContent = `${id} | top:${top} h:${height} left:${left} w:${width}`;
  ...
  // Keep overlay aligned on resize/orientation/scroll
  window.addEventListener("resize", () => { if (vcHitboxDebugOn) refreshHitboxDebug(); }, { passive: true });
  window.addEventListener("orientationchange", () => { if (vcHitboxDebugOn) setTimeout(refreshHitboxDebug, 50); }, { passive: true });
  document.addEventListener("scroll", () => { if (vcHitboxDebugOn) refreshHitboxDebug(); }, { passive: true });
