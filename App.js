...
function clearNode(node) {
  ...
  // ---------- Story Cover Resolution ----------
  function resolveStoryCoverUrl(storyMeta) {
    if (!storyMeta || !storyMeta.cover || !storyMeta.cover.path) return null;

    let raw = String(storyMeta.cover.path).trim();
    if (!raw) return null;
    ...
    // normalize to covers folder under pack
    const file = raw.includes("/") ? raw : "covers/" + raw;
    if (state.packBaseUrl) {
      return new URL(file, state.packBaseUrl).toString();
    }

    // fallback: content root
    return new URL("./content/" + file.replace(/^\.\//,""), location.href).toString();
  }
// ---------- Rendering ----------
