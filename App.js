...
function clearNode(node) {
  ...
  // ---------- Menu Skin Preload (v0.1.42) ----------
  function preloadMenuSkins() {
    try {
      [MENU_SKIN_NOSAVE_URL, MENU_SKIN_SAVE_URL].forEach((url) => {
        const img = new Image();
        img.onload = () => log(`Menu skin loaded: ${url}`);
        img.onerror = () => log(`Menu skin FAILED: ${url}`);
        img.src = url;
      });
    } catch (e) {
...
function init() {
    ...
    // Enter Menu immediately so scroll-lock + gesture guards take effect before any user interaction.
    setScreen("menu");
    // Apply correct menu art based on save state and warm both images.
    setMenuSkinBySaveState();
    preloadMenuSkins();
