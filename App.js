...
function clearNode(node) {
  ...
  function emitEcho(msg) {
    if (!msg) return;
    const line = String(msg);
    state.echoQueue.push(line);
    log(`ECHO + ${line}`);
  }
  ...
  function renderEcho() {
    ...
    log(`ECHO render (${state.echoQueue.length})`);
    state.echoQueue.length = 0;
  }
...
const title = state.selectedStoryMeta?.title || state.storyData?.title || "Story";
    ...
    if (typeof rawEcho === "string") emitEcho(rawEcho);
    else if (Array.isArray(rawEcho)) rawEcho.forEach(emitEcho);
