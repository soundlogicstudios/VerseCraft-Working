...
// ---------- Modal ----------
  ...
  function navTo(screen, reason, opts = {}) {
    ...
    // Any forward navigation dismisses an open modal first.
    closeModalIfOpen(reason || "navTo");
    ...
    // Optional: mirror navigation into browser history so hardware back works.
    if (!opts.noHistory) pushNavHistory(next, replace);

    log(`NAV: ${next} ← ${reason || "navTo"} stack=${stackToString()}`);
  }

  function navBack(fallback, reason) {
    // Back closes modal first (Phase 4).
    if (closeModalIfOpen(reason || "navBack")) return;
  ...
  // ---------- Phase 4: Optional Browser History Wiring ----------
  // Keeps iOS/Android browser back button aligned with the in-app screen stack.
  // Safe: if history is unavailable or user entered deep links, we fall back to navBack.
  function pushNavHistory(screenName, replace) {
    try {
      if (!window.history || !window.history.pushState) return;
      const stateObj = { vc: 1, screen: String(screenName || "menu") };
      if (replace) window.history.replaceState(stateObj, "");
      else window.history.pushState(stateObj, "");
    } catch {}
  }

  function installPopstateNav() {
    try {
      window.addEventListener("popstate", () => {
        // Close modal first, otherwise pop navigates back.
        if (closeModalIfOpen("popstate")) return;
        navBack("menu", "popstate");
      });
    } catch {}
  }
  ...
  // ---------- Phase 4: Screen Inerting + Scroll Reset ----------
  // iOS Safari can "ghost" inactive screens (tap interception / layout bleed) if any element
  // remains in the render tree. We enforce: hidden + aria-hidden + (optional) inert + pointer-events.
  function setScreenInteractivity(node, isActive) {
    if (!node) return;
    try {
    ...
    // Accessibility + tap safety
    node.setAttribute("aria-hidden", isActive ? "false" : "true");
    node.style.pointerEvents = isActive ? "auto" : "none";

    // Best-effort inert (not universally supported, but safe to set)
    try {
  ...
  function resetScreenScroll(name) {
    // Always reset the root app scroller (prevents partial return states).
    if (el.screensRoot) el.screensRoot.scrollTop = 0;

    // Also reset any dedicated inner scrollers used by specific screens.
    const s = el.screens[name];
    if (!s) return;
    const inner = s.querySelectorAll(".catalog-scroll, .stories-scroll, .debug-body");
    inner.forEach((n) => {
      try { n.scrollTop = 0; } catch {}
    });
  }

  function modalIsOpen() {
    return !!(el.modalBackdrop && el.modalBackdrop.hidden === false);
  }

  function closeModalIfOpen(reason) {
    if (!modalIsOpen()) return false;
    hideModal();
    log(`Modal closed ← ${reason || "back"}`);
    return true;
  }
...
function setScreen(name) {
    ...
    // Deactivate all screens hard (hidden + inert) to prevent tap interception / bleed.
    Object.entries(el.screens).forEach(([key, node]) => {
      setScreenInteractivity(node, key === next);
    });

    currentScreen = next;

    // Clear any pending echo UI when leaving the game screen.
    if (next !== "game") clearEcho();

    // Global scroll lock ONLY while Menu is active (prevents rubber-band/parallax on the skin)
    setGlobalScrollLock(next === "menu");

    // Menu active class gates .screens overflow in CSS
    const body = document.body;
    if (body) body.classList.toggle(MENU_ACTIVE_CLASS, next === "menu");

    // Reset scroll positions on every navigation.
    resetScreenScroll(next);
