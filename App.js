...
function init() {
    ...
    // Phase 4A: Screen Skin Hooks (non-menu) - toggle overlay class when bg vars are set
    function applyScreenBgFlags() {
      const rootStyle = getComputedStyle(document.documentElement);
      const map = [
        ["catalog", "--vc-bg-catalog-url"],
        ["stories", "--vc-bg-stories-url"],
        ["settings", "--vc-bg-settings-url"],
        ["game", "--vc-bg-game-url"],
      ];
      map.forEach(([screen, varName]) => {
        const node = el.screens[screen];
        if (!node) return;
        const v = (rootStyle.getPropertyValue(varName) || "").trim();
        const has = v && v !== "none";
        node.classList.toggle("vc-has-bg", has);
        if (DEBUG) log(`BG: ${screen} = ${has ? v : "none"}`);
      });
    }
    applyScreenBgFlags();
