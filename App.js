...
function openEquipPicker(slot) {
  const inv = app.state?.inventory ?? {};
  const items = Array.isArray(inv.items) ? inv.items : [];
  if (!items.length) return toast("No items available to equip yet.");

  // Filter items by catalog allowedSlots (prevents cross-contamination)
  const filtered = items.filter(id => isItemAllowedInSlot(id, slot));
  ...
  const list = filtered.map(id => {
    ...
    return `<button class="ChoiceBtn" type="button" data-equip-id="${escapeHtmlAttr(id)}">${escapeHtml(name)}</button>`;
  }).join("");

  openModal(el, {
    title: `Equip to ${slotLabel}`,
    bodyHtml: `<div class="Hint">Choose an item:</div>
...
function isItemAllowedInSlot(id, slot) {
  const def = app.story?.items?.[id];
  if (!def || typeof def !== "object") return false; // strict: unknown items cannot be equipped
  if (!Array.isArray(def.allowedSlots)) return false;
  return def.allowedSlots.includes(slot);
}
...
function doEquip(slot, id) {
  if (!isItemAllowedInSlot(id, slot)) {
    toast("That item doesn't fit that slot.");
    return;
  }
  app.state = applyEffects(app.state, [{ op: "equip", slot, id }], app.story);
  renderCurrent();
}

function doUnequip(slot) {
  app.state = applyEffects(app.state, [{ op: "unequip", slot }], app.story);
  renderCurrent();
}

// --- saves modal etc (unchanged) ---
