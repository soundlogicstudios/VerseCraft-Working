...
(() => {
  ...
  function clearSave() {
    // Backward-safe: clear any known save keys from older builds
    const keysToClear = [
      SAVE_KEY,
      "versecraft.save.v1",
      "versecraft.save",
    ...
    const hadAny = keysToClear.some((k) => {
      try { return !!localStorage.getItem(k); } catch { return false; }
    });

    if (!hadAny && !state.save) {
      showModal("No Save Found", "There is no saved game to clear. (It has already been cleared.)");
      return;
    }
    ...
    // Clear in-memory progress as well to prevent immediate re-save on next action
    state.save = null;
    state.storyData = null;
    state.storyUrl = null;
    state.sectionId = null;
    state.selectedStoryMeta = null;
