...
async function copyDebugToClipboard() {
  ...
  // ---------- Debug Tools (Copy / Snapshot) ----------
  function getStateSummary() {
    const scr = currentScreen || "—";
    const pack = state.selectedPack?.packId || state.selectedPack?.id || state.selectedPack?.title || "—";
    const story = state.selectedStoryMeta?.storyId || state.selectedStoryMeta?.title || state.storyData?.title || "—";
    const section = state.sectionId || "—";
    const url = state.storyUrl || "—";
    const save = state.save ? `${state.save.storyTitle || state.save.storyId || "Story"} §${state.save.sectionId}` : "None";
    const rt = state.runtime
      ? `hp=${state.runtime.hp ?? "—"}/${state.runtime.hpMax ?? "∞"} xp=${state.runtime.xp ?? 0} flags=${Object.keys(state.runtime.flags || {}).length}`
      : "—";
    return { screen: scr, pack, story, section, storyUrl: url, save, runtime: rt };
  }
  ...
  function buildDebugDump() {
    ...
    const vv = window.visualViewport;
    const vvStr = vv
      ? `${Math.round(vv.width)}x${Math.round(vv.height)} offset(${Math.round(vv.offsetLeft || 0)}, ${Math.round(vv.offsetTop || 0)}) scale(${vv.scale || 1})`
      : "n/a";
    const vpStr = `${window.innerWidth}x${window.innerHeight}`;
    const dpr = window.devicePixelRatio || 1;

    // Grab last ~200 lines from visible debug log (newest-first in DOM)
    let lines = [];
    try {
      const nodes = el.debugBody ? Array.from(el.debugBody.querySelectorAll(".debug-line")) : [];
      lines = nodes.slice(0, 200).map((n) => n.textContent || "").filter(Boolean);
    } catch {}
...
`;
  ...
  function saveSnapshotToStorage(payload) {
    try {
      const key = "versecraft.debug.snapshots.v1";
