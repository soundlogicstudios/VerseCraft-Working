...
function applyStoryLoadoutOnce(storyData) {
  ...
  function initCosmeticsSettingsUi() {
    ...
    const applyBtn = document.getElementById("btnApplyTheme");
    const status = document.getElementById("cosmeticsStatus");
    if (!select || !applyBtn || !status || !cosmeticsState.catalog) return;
    ...
    select.value = cosmeticsState.activeThemeId || (cosmeticsState.catalog.defaults && cosmeticsState.catalog.defaults.activeThemeId) || "theme_parchment";

    const updateStatus = () => {
      const t = themes.find(x => x.id === select.value);
      const name = t ? (t.displayName || t.id) : select.value;
    ...
    applyBtn.addEventListener("click", () => {
      applyThemeById(select.value);
      updateStatus();
      showToast(`Theme Applied: ${select.options[select.selectedIndex].textContent}`);
    });
  }
