...
// ---------- Modal ----------
  ...
  function setScreen(name) {
    ...
    setGlobalScrollLock(name === "menu" || name === "settings");
    ...
    if (body) {
      ...
      body.classList.toggle("vc-settings-active", name === "settings");
    }
...
function renderGame() {
  ...
  function wireUi() {
    ...
    // modal
    // iOS Safari: prefer touchend for more reliable hit detection (some overlays can swallow click)
    const onModalClose = (e) => {
      if (e) { e.preventDefault(); e.stopPropagation(); }
      hideModal();
    };
    el.btnModalClose.addEventListener("click", onModalClose);
    el.btnModalClose.addEventListener("touchend", onModalClose, { passive: false });

    // Tap outside modal closes
    el.modalBackdrop.addEventListener("click", (e) => {
      if (e.target === el.modalBackdrop) hideModal();
    });
    el.modalBackdrop.addEventListener("touchend", (e) => {
      if (e.target === el.modalBackdrop) { e.preventDefault(); hideModal(); }
    }, { passive: false });
