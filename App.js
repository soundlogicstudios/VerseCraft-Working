...
(() => {
  ...
  function setPillboxStory(title) {
    el.pillMode.textContent = `Story: ${title}`;
    el.pillContext.textContent = "—";
    pulsePillbox();
  }
...
function clearNode(node) {
  ...
  async function continueFromSave() {
    ...
    try {
      ...
      setScreen("game");
      renderGame();
      setPillboxStory(s.storyTitle || storyData.title || "Story");
      log("Continue OK");
    } catch (e) {
      clearSave();
  ...
  async function startStory(storyMeta) {
    try {
      ...
      setScreen("game");
      setPillboxStory(storyMeta.title || state.storyData.title || "Story");
      renderGame();
  ...
  const MENU_TEMPLATE_W = 1024;
  const MENU_TEMPLATE_H = 1536;

  // Button rectangles in template pixel space (x, y, w, h)
  // These are intentionally approximate — tuned for the provided template.
  const MENU_HITBOXES = {
    // Tuned via on-device calibration (v0.1.17)
    btnLoadNew: { x: 0.08, y: 0.350, w: 0.84, h: 0.090 },
    btnContinue:{ x: 0.08, y: 0.460, w: 0.84, h: 0.090 },
    btnSettings:{ x: 0.08, y: 0.570, w: 0.84, h: 0.090 },
  };

  function preloadMenuSkin() {
    try {
      // Make hitboxes clickable immediately (even if image is slow)
      setTimeout(positionMenuHitboxes, 0);

      const imgEl = document.querySelector("#screen-menu .vc-menu-img");
      if (imgEl) {
        imgEl.onload = () => {
          log(`Menu skin loaded: ${MENU_SKIN_URL}`);
          positionMenuHitboxes();
        };
        imgEl.onerror = () => {
          log(`Menu skin FAILED: ${MENU_SKIN_URL}`);
          // Keep viewport-based hitboxes so the menu remains usable
          positionMenuHitboxes();
        };
        imgEl.src = MENU_SKIN_URL;

        // If cached, onload may not fire on some iOS paths—handle immediately.
        if (imgEl.complete) {
          log(`Menu skin loaded (cached): ${MENU_SKIN_URL}`);
          positionMenuHitboxes();
        }
      } else {
        const img = new Image();
  ...
  function getMenuImageRect() {
    const imgEl = document.querySelector("#screen-menu .vc-menu-img");
    if (!imgEl) return null;
    const vw = imgEl.clientWidth;
    const vh = imgEl.clientHeight;

    // object-fit: contain => rendered image is centered with possible letterboxing.
    const imgAspect = MENU_TEMPLATE_W / MENU_TEMPLATE_H;
    const viewAspect = vw / vh;

    let rw, rh, rx, ry;
    if (viewAspect > imgAspect) {
      // view wider => image constrained by height
      rh = vh;
      rw = rh * imgAspect;
      rx = (vw - rw) / 2;
      ry = 0;
    } else {
      // view taller => image constrained by width
      rw = vw;
      rh = rw / imgAspect;
      rx = 0;
      ry = (vh - rh) / 2;
  ...
  function positionMenuHitboxes() {
    const r = getMenuImageRect() || getViewportRect();
    // If we are still before layout, fall back to viewport.
    if (!r || r.width < 50 || r.height < 50) return;

    Object.keys(MENU_HITBOXES).forEach((id) => {
      const btn = document.getElementById(id);
      const box = MENU_HITBOXES[id];
      if (!btn || !box) return;

      btn.style.left = (r.left + (r.width * box.x)) + "px";
      btn.style.top = (r.top + (r.height * box.y)) + "px";
      btn.style.width = (r.width * box.w) + "px";
      btn.style.height = (r.height * box.h) + "px";
    });
  }

  function preventMenuScroll() {
    try {
      // Prevent iOS rubber-band scroll on the menu background
      const el = document.getElementById("screen-menu");
      if (!el) return;
      el.addEventListener("touchmove", (e) => {
        // Allow normal scrolling nowhere on menu (menu is full-screen)
        e.preventDefault();
      }, { passive: false });
    } catch (_) {}
  }
...
function init() {
    ...
    preloadMenuSkin();
    preventMenuScroll();
    setTimeout(positionMenuHitboxes, 50);
    setTimeout(positionMenuHitboxes, 250);
    window.addEventListener('resize', positionMenuHitboxes, { passive: true });
    window.addEventListener('orientationchange', positionMenuHitboxes, { passive: true });
    preloadMenuSkin();
    preventMenuScroll();
    setTimeout(positionMenuHitboxes, 50);
    setTimeout(positionMenuHitboxes, 250);
