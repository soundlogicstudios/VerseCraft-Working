...
function clearNode(node) {
  ...
  function resolveCoverUrl(storyMeta) {
      // Canon preferred: story.cover.path (object form)
      // Backward compatible: story.cover as string, story.coverPath, story.cover_url, story.coverUrl
      if (!storyMeta) return null;
  
      let rawPath = null;
  
      // Preferred
      if (storyMeta.cover && typeof storyMeta.cover === "object") {
        rawPath = storyMeta.cover.path || storyMeta.cover.url || null;
      } else if (typeof storyMeta.cover === "string") {
        rawPath = storyMeta.cover;
      }
  
      // Back-compat aliases
      rawPath = rawPath || storyMeta.coverPath || storyMeta.cover_url || storyMeta.coverUrl || null;
      ...
      // If it's just "file.webp", map to "covers/file.webp"
      const rel = raw.includes("/") ? raw : ("covers/" + raw);
  
      // Pack-relative base (preferred)
      if (state.packBaseUrl) return new URL(rel, state.packBaseUrl).toString();
  ...
  function renderStories() {
    ...
    stories.forEach((s) => {
      ...
      // Background cover (Story List) — uses Pack.json cover metadata
      const coverUrl = resolveCoverUrl(s);
      if (coverUrl) {
        card.classList.add("has-cover");
        card.style.setProperty("--vc-story-cover", `url("${coverUrl}")`);
        card.style.backgroundImage = `url("${coverUrl}")`;

        // Debug-only verification (requires ?debug=1)
        if (DEBUG) {
          log(`Cover URL = ${coverUrl}`);
          try {
            const img = new Image();
            img.onload = () => log(`Cover OK: ${coverUrl}`);
            img.onerror = () => log(`Cover FAIL: ${coverUrl}`);
            img.src = coverUrl;
          } catch (e) {
            log(`Cover preload error: ${String(e)}`);
          }
        }
      } else {
        if (DEBUG) log(`Cover missing: ${s.title || s.storyId || "Story"}`);
      }
const title = document.createElement("h3");
  ...
  function getSectionfunction getSection(storyData, sectionId) {
...
function init() {
    ...
    if (DEBUG) log("Build: v1.44.3-covercards-backcompat");
    preloadMenuSkin();
