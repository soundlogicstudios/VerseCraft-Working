...
(() => {
  ...
  function syncVersionUI() {
    try {
      ...
      const footer = document.getElementById("appVersion");
      if (footer) footer.textContent = v;
      if (el.versionBadge) el.versionBadge.textContent = v;
    } catch {}
  }
  ...
  // ---------- iOS Scroll Lock (menu only) ----------
  // Goal: prevent iOS rubber-band / parallax scroll on the Menu screen
  // while keeping scroll available for catalog/stories/game screens.
  ...
  // Prevent touch-driven scroll on iOS when menu is locked.
  document.addEventListener(
    "touchmove",
    (e) => {
      if (document.body && document.body.classList.contains(SCROLL_LOCK_CLASS)) {
        e.preventDefault();
      }
    },
    { passive: false }
