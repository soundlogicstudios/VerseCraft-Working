  ...
  function renderGame() {
    ...
    choices.forEach((c) => {
      ...
      btn.addEventListener("click", () => {
        // Phase 3D.3: return routing (allows choice without destination)
        if (c.toMenu === true || c.toMainMenu === true) {
          writeSave();
          if (DEBUG) log("ROUTE: toMenu");
          setScreen("menu");
          return;
        }
        if (c.toStories === true) {
          writeSave();
          if (DEBUG) log("ROUTE: toStories");
          setScreen("stories");
          return;
        }
