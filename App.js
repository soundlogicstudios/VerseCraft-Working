...
updateContinueButton();
  ...
  // Global Gesture Guards (iOS Safari)
  // Goal: prevent pinch-to-zoom across the app while preserving intentional in-app scrolling.
  // iOS Safari emits non-standard gesture* events; blocking them is the most reliable.
  function setupGlobalGestureGuards() {
    // Block iOS "gesture" events everywhere (pinch/zoom).
    ["gesturestart", "gesturechange", "gestureend"].forEach((type) => {
      document.addEventListener(
        type,
        (e) => {
    ...
    // Block Ctrl+wheel zoom (desktop Safari/macOS) just in case.
    document.addEventListener(
      "wheel",
      (e) => {
