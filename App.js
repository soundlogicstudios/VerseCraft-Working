...
// ---------- Debug ----------
  ...
  function setScreen(name) {
    ...
    log(`Screen → ${name}${prevScreen ? " (prev: " + prevScreen + ")" : ""}`);
    if (DEBUG) log(`STATE: ${debugStateSummary()}`);
    }
...
function clearNode(node) {
  ...
  async function fetchJson(url) {
    ...
    if (DEBUG) log(`FETCH start: ${abs}`);

    let res;
    try {
      res = await fetch(cacheBuster(url), { cache: "no-store" });
    } catch (e) {
      const ms = ((performance && performance.now) ? performance.now() : Date.now()) - t0;
      state.lastFetch = { url: abs, ms: Math.round(ms), ok: false, status: 0, error: "network" };
      if (DEBUG) log(`FETCH FAIL (network) (${Math.round(ms)}ms): ${abs}`);
      throw e;
    }
    ...
    if (!res.ok) {
      state.lastFetch = { url: abs, ms: Math.round(ms), ok: false, status: res.status, error: "http" };
    ...
    try {
      ...
      state.lastFetch = { url: abs, ms: Math.round(ms), ok: false, status: res.status, error: "parse" };
      if (DEBUG) log(`JSON PARSE ERROR (${Math.round(ms)}ms): ${abs}`);
      throw new Error(`JSON parse error for ${new URL(url, location.href).pathname}`);
    }

    state.lastFetch = { url: abs, ms: Math.round(ms), ok: true, status: res.status, contentType: ct };
    if (DEBUG) log(`FETCH OK (${Math.round(ms)}ms)${ct ? " · " + ct : ""}: ${abs}`);
    return data;
  }
  ...
  // ---------- Phase 3A: Debug State Summary (Debug-only) ----------
  function debugStateSummary() {
    if (!DEBUG) return "";
    const packId = state.selectedPack?.packId || state.selectedPack?.id || state.selectedPack?.title || "—";
    const storyId = state.selectedStoryMeta?.storyId || state.selectedStoryMeta?.id || "—";
    const sec = state.sectionId || "—";
    const last = state.lastFetch?.url ? `lastFetch=${state.lastFetch.url} (${state.lastFetch.ms}ms)` : "lastFetch=—";
    return `screen=${currentScreen} pack=${packId} story=${storyId} section=${sec} · ${last}`;
  }
...
// ---------- Rendering ----------
  function renderCatalog() {
    ...
    if (DEBUG) log(`Catalog render OK: ${Array.isArray(packs) ? packs.length : 0} pack(s)`);
  ...
  function renderStories() {
    ...
    if (DEBUG) log(`Stories render OK: pack=${pack?.packId || pack?.id || pack?.title || "?"} · ${Array.isArray(stories) ? stories.length : 0} story(s)`);
  ...
  function renderGame() {
    ...
    if (DEBUG) log(`Game render: ${state.storyData?.title || title} · section=${state.sectionId}`);
    ...
    if (DEBUG) log(`Game render OK: ${Array.isArray(choices) ? choices.length : 0} choice(s)`);
