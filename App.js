...
(() => {
  ...
  // Menu background art (lowercase, canonical)
    const MENU_SKIN_SAVE_URL = './backgrounds/ui/VerseCraft-Menu_v1.png';
    const MENU_SKIN_NOSAVE_URL = './backgrounds/ui/VerseCraft-Menu_v1.png';
...
// ---------- Modal ----------
  function showModal(title, text) {
    ...
    // iOS Safari can occasionally keep a hidden element in the hit-test tree if
    // visibility/display was previously manipulated. Force a known-good state.
  ...
  function isBackdropTap(evt) {
    // Only treat taps/clicks on the backdrop itself as "outside" taps.
    return evt && evt.target === el.modalBackdrop;
  }
...
function renderGame() {
  ...
  function wireUi() {
    ...
    // iOS Safari reliability: ensure modal can always be dismissed
    el.btnModalClose.addEventListener(
      "touchstart",
      (e) => {
        e.preventDefault();
        hideModal();
