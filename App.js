...
function applyStoryLoadoutOnce(storyData) {
  ...
  function renderCharacterScreen() {
    if (!el.screens.character) return;
    ...
    // Labels
    if (el.charStoryLabel) el.charStoryLabel.textContent = story ? `${story.title || "Story"}${state.sectionId ? " • " + state.sectionId : ""}` : "—";

    // Slot labels
    const labels = mod.slotLabels || {};
    if (el.slotLabelWeapon) el.slotLabelWeapon.textContent = labels.weapon || "Weapon";
    if (el.slotLabelArmor) el.slotLabelArmor.textContent = labels.armor || "Armor";
    if (el.slotLabelSpecial) el.slotLabelSpecial.textContent = labels.special || "Special";

    // HP meter
    if (r) {
      if (el.charHpLabel) el.charHpLabel.textContent = `${pr.name}: ${r.cur}/${r.max}`;
      const pct = r.max ? Math.max(0, Math.min(100, (Number(r.cur) / Number(r.max)) * 100)) : 0;
      if (el.charHpFill) el.charHpFill.style.width = `${pct}%`;
    } else {
      if (el.charHpLabel) el.charHpLabel.textContent = `${pr.name}: —`;
      if (el.charHpFill) el.charHpFill.style.width = "0%";
    }

    // XP / Currency
    if (el.charXp) el.charXp.textContent = String(Number(state.run.xp || 0));
    if (el.charCurrency) {
      const c = (state.run.currency != null) ? state.run.currency : "—";
      el.charCurrency.textContent = String(c);
    }

    // Equipment
    const eq = state.run.inventory?.equipped || {};
    if (el.charEquipWeapon) el.charEquipWeapon.textContent = eq.weapon ? getDisplayNameForItem(eq.weapon) : "—";
    if (el.charEquipArmor) el.charEquipArmor.textContent = eq.armor ? getDisplayNameForItem(eq.armor) : "—";
    if (el.charEquipSpecial) el.charEquipSpecial.textContent = eq.special ? getDisplayNameForItem(eq.special) : "—";

    // Inventory list
    if (el.charInventoryList) {
      el.charInventoryList.innerHTML = "";
      const items = (state.run.inventory?.items || []).slice(0);

      if (!items.length) {
        const p = document.createElement("p");
        p.className = "subtle";
        p.textContent = "Inventory empty.";
        el.charInventoryList.appendChild(p);
      } else {
        items.forEach((it) => {
          if (!it || !it.id) return;
          const def = getItemDef(it.id);
          const name = def?.name || it.title || it.id;
          const meta = [];
          if (it.category) meta.push(String(it.category));
          if (Number.isFinite(it.qty) || it.qty != null) meta.push(`x${it.qty}`);
          const equipSlot = inferEquipSlot(it.id, it);
          const canUse = !!def?.use;
          ...
          n.className = "inv-name";
          n.textContent = String(name);
          const mm = document.createElement("div");
          mm.className = "inv-meta";
          ...
          if (equipSlot) {
            ...
            b.textContent = `Equip`;
            b.addEventListener("click", () => equipItemToSlot(equipSlot, it.id));
            actions.appendChild(b);
          }

          if (canUse && Number(it.qty || 0) > 0) {
            const b2 = document.createElement("button");
            b2.className = "btn-mini";
            b2.textContent = "Use";
            b2.addEventListener("click", () => useConsumable(it.id));
            actions.appendChild(b2);
          }

          row.appendChild(main);
          row.appendChild(actions);
          el.charInventoryList.appendChild(row);
        });
      }
    }
    ...
    // Flags
    if (el.charFlagsList) {
      el.charFlagsList.innerHTML = "";
      const flags = state.run.flags || {};
      const on = Object.keys(flags).filter(k => flags[k]).sort();
      if (!on.length) {
        const p = document.createElement("p");
        p.className = "subtle";
        p.textContent = "No flags set.";
        el.charFlagsList.appendChild(p);
      } else {
        on.forEach((k) => {
          const pill = document.createElement("span");
          pill.className = "flag-pill";
          pill.textContent = k;
          el.charFlagsList.appendChild(pill);
        });
      }
    }
  }
...
function renderCharacterScreen() {
  ...
  const hpVal = (state.run && state.run[prKey] != null) ? Number(state.run[prKey]) : Number(state.run?.hp ?? pr.max);
  const hpMax = (state.run && state.run[prKey + "Max"] != null) ? Number(state.run[prKey + "Max"]) : Number(state.run?.hpMax ?? pr.max);

  const xpKey = (sd?.module?.xpKey) ? String(sd.module.xpKey) : "xp";
  const xpLabel = getScalarLabel(sd, "xp", "XP");
  const xpVal = Number(state.run?.[xpKey] || 0);

  // Currency is optional; creator-defined. Default key "currency".
  const curKey = (sd?.module?.currencyKey) ? String(sd.module.currencyKey) : "currency";
  const curLabel = getScalarLabel(sd, "currency", "Currency");
  const curVal = (state.run && state.run[curKey] != null) ? state.run[curKey] : "—";

  // Equipment map is dynamic
  state.run.inventory = state.run.inventory || { items: [], equipped: {} };
  state.run.inventory.equipped = state.run.inventory.equipped || {};

  // Render header stats
  const statsEl = $("charStats");
  if (statsEl) {
    statsEl.innerHTML = `
      <div class="kv"><div class="k">${escapeHtml(prLabel)}</div><div class="v">${hpVal}/${hpMax}</div></div>
  ...
  // Render slots
  const slotsEl = $("charSlots");
  if (slotsEl) {
    slotsEl.innerHTML = "";
    slotDefs.forEach(def => {
      const equippedId = state.run.inventory.equipped[def.key] || "—";
      const row = document.createElement("div");
      row.className = "slot-row";
      row.innerHTML = `
        <div class="slot-label">${escapeHtml(def.label)}</div>
        <div class="slot-value">${escapeHtml(String(equippedId))}</div>
      `;
      slotsEl.appendChild(row);
    });
  }

  // Render inventory list (equip/use)
  const invEl = $("charInventory");
  if (invEl) {
    invEl.innerHTML = "";
    ...
    items
      ...
      .forEach(it => {
        ...
        const title = cat.title || cat.name || it.title || it.id;
        ...
        // Equip button if item declares a slot that exists in story slotDefs
        if (equipSlot) {
        ...
        // Use button for consumables (simple tutorial behavior)
        if (isConsumable) {
          const btn = document.createElement("button");
          btn.className = "btn btn-utility";
          btn.textContent = "Use";
          btn.addEventListener("click", () => {
            // Apply any "onUse" effects if present; otherwise no-op except consume 1
            const onUse = cat.onUse || cat.effectsOnUse || null;
            if (onUse) applyEffects(onUse, { source: "consume", itemId: it.id });
            removeItem(it.id, 1);
            emitEcho(`Used: ${title}`, true);
            renderEcho();
            saveRun();
            renderCharacterScreen();
          });
        ...
        card.innerHTML = `
          <div class="item-title">${escapeHtml(title)} <span class="item-qty">x${Number(it.qty||0)}</span></div>
