...
(() => {
  ...
  // Hardening Phase 1: boot heartbeat
  window.__VC_BOOTED__ = true;

  const ENGINE_VERSION = "0.1.44";
...
// ---------- Debug ----------
  ...
  const __vcLogBuffer = [];
  if (DEBUG) console.log("VerseCraft Debug enabled");

  function log(msg) {
    if (!DEBUG) return;
    const ts = new Date().toLocaleTimeString();
    const safe = (typeof msg === "string") ? msg : (() => { try { return JSON.stringify(msg); } catch { return String(msg); } })();
    __vcLogBuffer.push(`[${ts}] ${safe}`);
    const line = document.createElement("div");
    line.className = "debug-line";
    line.innerHTML = `<span class="debug-ts">[${ts}]</span>${escapeHtml(safe)}`;
  ...
  // ---------- Hardening Phase 1: Global Error Traps (Debug-only) ----------
  window.addEventListener("error", (e) => {
    if (!DEBUG) return;
    try {
      const file = e.filename ? e.filename.split("/").slice(-1)[0] : "unknown";
      log(`JS ERROR: ${e.message || "Unknown"} @ ${file}:${e.lineno || "?"}:${e.colno || "?"}`);
    } catch {}
  });
  ...
  window.addEventListener("unhandledrejection", (e) => {
    ...
    try {
      ...
      log(`PROMISE REJECTION: ${String(reason)}`);
    } catch {}
  });


  // ---------- Hardening Phase 1.2: Debug Pill Fallback ----------
  // If the built-in debug toggle button is missing/hidden due to DOM/CSS issues,
  // create a minimal floating pill so ?debug=1 is never "blind" on iOS.
  function ensureDebugPill() {
    if (!DEBUG) return;
    try {
      if (el.btnDebugToggle) {
        el.btnDebugToggle.hidden = false;
        return;
      }
      if (document.getElementById("vcDebugPillFallback")) return;
      const pill = document.createElement("button");
      pill.id = "vcDebugPillFallback";
      pill.className = "vc-debug-pill";
      pill.type = "button";
      pill.textContent = "Debug";
      pill.setAttribute("aria-pressed", "true");
      pill.addEventListener("click", () => toggleDebug());
      document.body.appendChild(pill);
      log("Debug pill fallback created");
    } catch (e) {
      // last resort: console
  ...
  function isAllowedScrollTarget(target) {
    ...
    // Always allow interaction/scroll inside debug panel.
    if (node && node.closest && node.closest("#debugPanel")) return true;
  ...
  // ---------- Hardening Phase 1: DOM Sanity Check (Debug-only) ----------
  function debugDomSanityCheck() {
    if (!DEBUG) return;
    const requiredIds = [
      "screen-menu","screen-settings","screen-catalog","screen-stories","screen-game",
      "btnLoadNew","btnContinue","btnSettings",
      "btnCatalogBack","btnCatalogReload","catalogList",
      "btnStoriesBack","storiesPackLabel","storiesList",
      "btnGameBack","btnGameMenu","gameTitle","gameText","gameChoices",
      "debugPanel","debugBody","btnDebugClose","btnDebugToggle"
    ];
    const missing = requiredIds.filter(id => !document.getElementById(id));
    if (missing.length) log(`DOM MISSING: ${missing.join(", ")}`);
    else log("DOM OK");
  }
...
function clearNode(node) {
  ...
  async function fetchJson(url) {
    ...
    if (DEBUG) log(`FETCH → ${abs}`);
    const res = await fetch(cacheBuster(url), { cache: "no-store" });
    const ms = ((performance && performance.now) ? performance.now() : Date.now()) - start;
    if (!res.ok) {
      if (DEBUG) log(`FETCH FAIL HTTP ${res.status} (${Math.round(ms)}ms) → ${abs}`);
      throw new Error(`HTTP ${res.status} for ${new URL(url, location.href).pathname}`);
    }
    const data = await res.json();
    if (DEBUG) log(`FETCH OK (${Math.round(ms)}ms) → ${abs}`);
    return data;
  }
  ...
  function wireUi() {
    ...
    if (el.btnDebugCopy) {
      el.btnDebugCopy.addEventListener("click", async () => {
        try {
          const text = __vcLogBuffer.join("\n");
        ...
        // Fallback (iOS): temporary textarea selection
        try {
          const ta = document.createElement("textarea");
          ta.value = __vcLogBuffer.join("\n");
          ...
          log("Debug logs copied (fallback).");
        } catch (e) {
...
updateContinueButton();
    ...
    if (el.btnDebugToggle) {
      el.btnDebugToggle.hidden = !DEBUG;
      el.btnDebugToggle.classList.toggle("on", DEBUG);
    }
    if (el.debugPanel) el.debugPanel.hidden = !DEBUG;
...
function init() {
    if (DEBUG) log(`Boot start · ${HARDENING_STAGE}`);
    ...
    wireUi();
    if (DEBUG) log("UI wired OK");
    initSettings();
    if (DEBUG) log("Settings initialized");
    debugDomSanityCheck();
    ensureDebugPill();

    log(`Engine v${ENGINE_VERSION}`);
    if (DEBUG) log(`Build: ${HARDENING_STAGE}`);
    preloadMenuSkin();
    // Stay on menu at boot; Continue is enabled if a valid save exists
    setScreen("menu");
    if (DEBUG) log("Menu displayed");
  }
