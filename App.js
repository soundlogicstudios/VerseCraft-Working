    ...
    if (name === "menu") {
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          // Phase 1 shimmer: trigger AFTER menu layers are visible (Option A)
          maybeTriggerContinueShimmerOnMenuReturn();
        });
      });
    } else {
      if (body) body.classList.remove(MENU_PREP_CLASS);
...
function clearNode(node) {
  ...
  // ---------- Phase 1: Continue Shimmer (Option A) ----------
  // Plays ONLY on transition: NO SAVE → HAS SAVE, and ONLY when the Menu screen is shown.
  // Overlay-based, inert (pointer-events none), non-looping, auto-removed.
  function getHasSaveNow() {
    return Boolean(state.save && state.save.storyUrl && state.save.sectionId);
  }

  function getMenuHadSaveFlag() {
    return localStorage.getItem(MENU_SHIMMER_FLAG_KEY) === "1";
  }

  function setMenuHadSaveFlag(hasSave) {
    try {
      localStorage.setItem(MENU_SHIMMER_FLAG_KEY, hasSave ? "1" : "0");
    } catch {
      // If storage fails, do not crash. Shimmer is cosmetic.
    }
  }

  function playContinueShimmerOnce() {
    // Only play on menu screen.
    if (currentScreen !== "menu") return;

    // Avoid duplicates if something calls twice in quick succession.
    if (document.getElementById("vcContinueShimmer")) return;

    const layer = document.querySelector("#screen-menu .vc-menu-overlay");
    if (!layer) return;

    const shimmer = document.createElement("div");
    shimmer.id = "vcContinueShimmer";
    shimmer.className = "vc-continue-shimmer";
    shimmer.setAttribute("aria-hidden", "true");
    shimmer.style.backgroundImage = `url("${CONTINUE_SHIMMER_OVERLAY_URL}")`;

    layer.appendChild(shimmer);

    // Remove after the CSS animation finishes (buffered).
    window.setTimeout(() => {
      if (shimmer && shimmer.parentNode) shimmer.parentNode.removeChild(shimmer);
    }, 950);
  }

  function maybeTriggerContinueShimmerOnMenuReturn() {
    const hasSaveNow = getHasSaveNow();
    const hadSaveBefore = getMenuHadSaveFlag();

    // Option A: only NO SAVE → HAS SAVE.
    if (!hadSaveBefore && hasSaveNow) {
      playContinueShimmerOnce();
    }

    // Always sync the flag to current truth.
    setMenuHadSaveFlag(hasSaveNow);
  }
  ...
  function setMenuSkinBySaveState() {
    ...
    // Phase 1 shimmer trigger (Option A)
    if (currentScreen === "menu") maybeTriggerContinueShimmerOnMenuReturn();
  }
  ...
  function clearSave() {
    ...
    state.save = null;
    // Phase 1 shimmer: allow replay after a Clear Save
    setMenuHadSaveFlag(false);
  ...
  async function continueFromSave() {
    ...
    try {
      ...
      initRuntimeFromStoryModule(storyData);
  ...
  // ---------- Phase 3C.1: Runtime State (HP/XP/Flags/Inventory) ----------
  function getDefaultRuntime() {
    return {
      hp: null,               // number
      hpMin: 0,
      hpMax: null,
      hpFailureSectionId: null,
      xp: 0,
      flags: {},              // map flagName -> true
      inventory: {            // category -> { id -> qty }
        weapons: {},
        armor: {},
        special: {},
        consumables: {},
      ...
      slots: { weapon: null, armor: null, special: null },
      itemCatalog: null,      // module.itemCatalog reference (if present)
    };
  }

  function invEnsure(runtime, category) {
    if (!runtime.inventory[category]) runtime.inventory[category] = {};
    return runtime.inventory[category];
  }

  function invAdd(runtime, category, id, qty) {
    if (!category || !id) return;
    const bucket = invEnsure(runtime, category);
    const n = Number(qty ?? 1);
    bucket[id] = (Number(bucket[id] || 0) + n);
    if (bucket[id] <= 0) delete bucket[id];
  }

  function invHas(runtime, category, id, qty) {
    if (!category || !id) return false;
    const bucket = invEnsure(runtime, category);
    const n = Number(qty ?? 1);
    return Number(bucket[id] || 0) >= n;
  }

  function invRemove(runtime, category, id, qty) {
    if (!category || !id) return;
    const bucket = invEnsure(runtime, category);
    const n = Number(qty ?? 1);
    bucket[id] = (Number(bucket[id] || 0) - n);
  ...
  function flagSet(runtime, name) {
    if (!name) return;
    runtime.flags[name] = true;
  }
  function flagClear(runtime, name) {
    if (!name) return;
    delete runtime.flags[name];
  }
  function flagHas(runtime, name) {
    return !!runtime.flags[name];
  }

  function initRuntimeFromStoryModule(storyData) {
    state.runtime = getDefaultRuntime();

    const mod = storyData?.module || storyData?.Module || null;
    if (!mod) {
      if (DEBUG) log("Runtime init: no module present (using defaults)");
      return;
    }

    // primary resource (HP)
    const pr = mod.primaryResource || mod.PrimaryResource || null;
    if (pr && (pr.name === "HP" || pr.name === "hp" || pr.type === "HP")) {
      state.runtime.hpMin = Number(pr.min ?? 0);
      state.runtime.hpMax = (pr.max === undefined || pr.max === null) ? null : Number(pr.max);
      state.runtime.hpFailureSectionId = pr.failureSectionId || pr.failureSection || pr.failTo || null;

      // Start HP rule: if module provides starting value, use it; else start at max (if defined), else min.
      const startHp = pr.start ?? pr.starting ?? pr.value ?? null;
      if (startHp !== null && startHp !== undefined) {
        state.runtime.hp = Number(startHp);
      } else if (state.runtime.hpMax !== null) {
        state.runtime.hp = Number(state.runtime.hpMax);
      } else {
        state.runtime.hp = Number(state.runtime.hpMin);
      }
    }

    // item catalog for debug validation + later systems
    state.runtime.itemCatalog = mod.itemCatalog || mod.ItemCatalog || null;

    // loadout
    const loadout = mod.loadout || mod.Loadout || null;
    if (loadout) {
      // inventory categories
      const cats = ["weapons","armor","special","consumables","items"];
      cats.forEach((cat) => {
        const list = loadout[cat] || loadout[capitalize(cat)] || [];
        if (Array.isArray(list)) {
          list.forEach((it) => {
            if (!it) return;
            const id = it.id || it.itemId || it.key;
            const qty = it.qty ?? it.quantity ?? 1;
            if (id) invAdd(state.runtime, cat, id, qty);
          });
        }
      });

      // equipped slots
      if (loadout.slots && typeof loadout.slots === "object") {
        state.runtime.slots.weapon = loadout.slots.weapon ?? null;
        state.runtime.slots.armor = loadout.slots.armor ?? null;
        state.runtime.slots.special = loadout.slots.special ?? null;
      }
    }

    if (DEBUG) {
      log(`Runtime init OK: HP=${state.runtime.hp ?? "—"}/${state.runtime.hpMax ?? "∞"} (min ${state.runtime.hpMin}) XP=${state.runtime.xp}`);
      const invSummary = Object.entries(state.runtime.inventory)
        .map(([cat, map]) => {
          const keys = Object.keys(map || {});
          return `${cat}:${keys.length}`;
        })
        .join(" ");
      log(`Runtime inventory: ${invSummary}`);
    }

    // Debug-only: validate JSON references against module itemCatalog
    debugValidateLorecraftRefs(storyData);
  }

  function normalizeEffects(effects) {
    if (!effects) return [];
    // Effects may be an object (merged ops) or array of ops
  ...
  function applyEffects(effects) {
    if (!state.runtime) state.runtime = getDefaultRuntime();

    const ops = normalizeEffects(effects);
    if (ops.length === 0) return { triggeredFail: false };

    let hpBefore = state.runtime.hp;

    ops.forEach((op) => {
      if (!op || typeof op !== "object") return;

      // direct numeric deltas
      if (op.hp !== undefined) {
        const d = Number(op.hp || 0);
        if (state.runtime.hp === null || state.runtime.hp === undefined) state.runtime.hp = 0;
        state.runtime.hp = Number(state.runtime.hp) + d;
        if (state.runtime.hpMax !== null && state.runtime.hpMax !== undefined) {
          state.runtime.hp = Math.min(state.runtime.hp, Number(state.runtime.hpMax));
        }
        state.runtime.hp = Math.max(state.runtime.hp, Number(state.runtime.hpMin || 0));
        if (DEBUG) log(`APPLY EFFECTS: hp ${d >= 0 ? "+" : ""}${d} (${hpBefore} → ${state.runtime.hp})`);
        hpBefore = state.runtime.hp;
      }

      if (op.xp !== undefined) {
        const d = Number(op.xp || 0);
        state.runtime.xp = Number(state.runtime.xp || 0) + d;
        if (DEBUG) log(`APPLY EFFECTS: xp ${d >= 0 ? "+" : ""}${d} (→ ${state.runtime.xp})`);
      }

      // flags
      if (op.setFlag) {
        flagSet(state.runtime, String(op.setFlag));
        if (DEBUG) log(`APPLY EFFECTS: setFlag ${String(op.setFlag)}`);
      }
      if (op.clearFlag) {
        flagClear(state.runtime, String(op.clearFlag));
        if (DEBUG) log(`APPLY EFFECTS: clearFlag ${String(op.clearFlag)}`);
      }

      // inventory ops
      if (op.addItem && typeof op.addItem === "object") {
        const cat = op.addItem.category;
        const id = op.addItem.id;
        const qty = op.addItem.qty ?? 1;
        invAdd(state.runtime, cat, id, qty);
        if (DEBUG) log(`APPLY EFFECTS: addItem ${cat}/${id} x${qty}`);
      }

      if (op.removeItem && typeof op.removeItem === "object") {
        const cat = op.removeItem.category;
        const id = op.removeItem.id;
        const qty = op.removeItem.qty ?? 1;
        // Debug warning if removal references unknown id in itemCatalog
        debugWarnUnknownItemRef(cat, id, "removeItem");
        invRemove(state.runtime, cat, id, qty);
        if (DEBUG) log(`APPLY EFFECTS: removeItem ${cat}/${id} x${qty}`);
      }
    });

    // failover on HP reaching min (or below)
    const failId = state.runtime.hpFailureSectionId;
    const triggeredFail = (failId && state.runtime.hp !== null && Number(state.runtime.hp) <= Number(state.runtime.hpMin || 0));
    if (triggeredFail && DEBUG) {
      log(`HP FAILOVER: hp=${state.runtime.hp} → failureSectionId=${failId}`);
    }
    return { triggeredFail: !!triggeredFail, failTo: triggeredFail ? String(failId) : null };
  }

  // ---------- Phase 3C.1a: Debug-only JSON mismatch warnings (Lorecraft) ----------
  // Purpose: warn when story references item ids that are not in module.itemCatalog.
  function debugWarnUnknownItemRef(category, id, context) {
    if (!DEBUG) return;
    const cat = String(category || "").trim();
    const key = String(id || "").trim();
    if (!cat || !key) return;

    const catalog = state.runtime?.itemCatalog;
    if (!catalog || typeof catalog !== "object") return;

    const catBucket = catalog[cat] || catalog[capitalize(cat)] || null;
    if (!catBucket || typeof catBucket !== "object") return;

    // catalog categories may be array or map
    let exists = false;
    if (Array.isArray(catBucket)) {
      exists = catBucket.some((it) => (it && (it.id === key || it.itemId === key)));
    } else {
      exists = !!catBucket[key];
    }

    if (!exists) {
      log(`WARN unknown item reference (${context}): ${cat}/${key} (not found in module.itemCatalog)`);
      // Specific helpful hints for known Lorecraft mismatches
      if (cat === "special" && key === "candle_token") {
        log(`HINT: Lorecraft uses special/candle_stub in loadout; candle_token looks mismatched.`);
      }
      if (cat === "items" && key === "candle") {
        log(`HINT: Lorecraft candle appears as special/candle_stub, not items/candle.`);
      }
    }
  }

  function debugValidateLorecraftRefs(storyData) {
    if (!DEBUG) return;
    const mod = storyData?.module || storyData?.Module || null;
    const catalog = mod?.itemCatalog || mod?.ItemCatalog || null;
    if (!catalog) return;

    const sectionsObj = storyData.sections || storyData.Sections || storyData.nodes || storyData.Nodes;
    const sections = [];
    if (Array.isArray(sectionsObj)) {
      sections.push(...sectionsObj);
    } else if (sectionsObj && typeof sectionsObj === "object") {
      Object.keys(sectionsObj).forEach((k) => sections.push({ id: k, ...(sectionsObj[k] || {}) }));
    }
    ...
    sections.forEach((sec) => {
      ...
      choices.forEach((c) => {
        // requires.hasItem
        const req = c.requires || c.Requires || null;
        const hasItem = req?.hasItem || req?.HasItem || null;
        if (hasItem && typeof hasItem === "object") {
          const cat = hasItem.category;
          const id = hasItem.id;
          const sig = `req:${cat}/${id}`;
          if (!seen.has(sig)) { seen.add(sig); debugWarnUnknownItemRef(cat, id, "requires.hasItem"); }
        }
        // effects: removeItem/addItem
        const ops = normalizeEffects(c.effects || c.Effects);
        ops.forEach((op) => {
          if (op?.removeItem) {
            const cat = op.removeItem.category;
            const id = op.removeItem.id;
            const sig = `rem:${cat}/${id}`;
            if (!seen.has(sig)) { seen.add(sig); debugWarnUnknownItemRef(cat, id, "effects.removeItem"); }
          }
          if (op?.addItem) {
            const cat = op.addItem.category;
            const id = op.addItem.id;
            const sig = `add:${cat}/${id}`;
            if (!seen.has(sig)) { seen.add(sig); debugWarnUnknownItemRef(cat, id, "effects.addItem"); }
          }
        });
      });
    });

    if (DEBUG) log("Lorecraft ref scan complete (debug warnings may appear above).");
  }
...
// ---------- Rendering ----------
  ...
  function renderGame() {
    ...
    choices.forEach((c) => {
      ...
      btn.addEventListener("click", () => {
        ...
        // Phase 3C.2: Apply choice effects before navigation
        const eff = c.effects || c.Effects;
        const effResult = applyEffects(eff);
        
        if (!next) {
          showModal("Choice Error", "Choice is missing a destination (to/goto/goTo/next/target).");
          return;
        }
        const dest = (effResult && effResult.triggeredFail && effResult.failTo) ? effResult.failTo : next;
        state.sectionId = String(dest);
        renderGame();
        writeSave();
      });
  ...
  async function startStory(storyMeta) {
    try {
      ...
      initRuntimeFromStoryModule(state.storyData);
