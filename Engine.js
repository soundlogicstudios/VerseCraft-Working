...
export function validateStory(story) {
  ...
  if (story.items != null && typeof story.items !== "object") {
    ...
    for (const [id, def] of Object.entries(story.items)) {
      ...
      if (def.maxOwned != null && !Number.isFinite(Number(def.maxOwned))) {
        errors.push(`items['${id}'].maxOwned must be a number (if present).`);
      }
      if (def.stackable != null && typeof def.stackable !== "boolean") {
        errors.push(`items['${id}'].stackable must be boolean (if present).`);
      }
    }
  }
  ...
  for (const e of effects) {
    ...
    if (e.op === "statAdd") {
      ...
      if (typeof e.id === "string") {
        ...
        const ok = canAcquireItem(story, next, e.id);
        if (ok) next.inventory.items.push(e.id);
        // else: silently ignore (we can surface messaging later)
      }
...
function countOwned(state, id) {
  return getAllOwnedItemIds(state).filter(x => x === id).length;
}

function canAcquireItem(story, state, id) {
  const def = getItemDef(story, id);

  // If no catalog entry exists, default to allow (back-compat).
  if (!def) return true;

  const stackable = Boolean(def.stackable);

  // Default caps:
  // - non-stackable defaults to maxOwned=1
  // - stackable defaults to maxOwned=9999 (effectively unlimited for now)
  const maxOwnedRaw = def.maxOwned;
  const maxOwned = Number.isFinite(Number(maxOwnedRaw))
    ? Number(maxOwnedRaw)
    : (stackable ? 9999 : 1);

  const owned = countOwned(state, id);
...
function getAllowedSlotsForItem(story, id) {
  ...
  return allowed.filter(s => SLOT_KEYS.has(s));
}
...
function equipFromItemsPool(state, slot, id, story) {
  ...
  if (allowed !== null && !allowed.includes(slot)) return;
