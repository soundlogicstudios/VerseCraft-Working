...
export function createNewGameState(story) {
  // Minimal canonical state (expand later without breaking saves)
  return {
...
export function isChoiceAvailable(state, choice) {
  ...
  // require: { statGte: { WISDOM: 2 }, hasFlag: ["X"], hasItem: ["ITEM_ID"] }
...
export function applyEffects(state, effects = []) {
  // Effects v1 (minimal, safe):
  // { op:"statAdd", key:"HEALTH", value: -1 }
  // { op:"flagSet", key:"FLAG_NAME", value:true }
  // { op:"giveConsumable", id:"WATER" }
  // Unknown ops are ignored (future-proof, but you can flip to "error" later)
  ...
  for (const e of effects) {
    ...
    if (e.op === "statAdd") {
      ...
      if (typeof e.id === "string") next.inventory.consumables.push(e.id);
    }
  }
